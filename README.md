...
🚧 **Import a book, open it, hear a real chapter** — navigation shell (Library / Voices / Settings) is wired up (see ADR-0004, ADR-0005), and every screen from `docs/design-reference/` is built and navigable. Sign In / Sign Up do real email/password auth against a live Supabase project (ADR-0006); Apple/Google are still placeholders. A signed-out user is gated to Onboarding — Library/Player/Settings/etc. all require a real session (ADR-0008). Voices/Voice Detail show a real ElevenLabs catalog with a real "Play preview," and "Set as default" actually changes what narrates playback (ADR-0009). The "+"/empty-state CTA on Library pick a real EPUB, parse it, and save it via the backend (ADR-0010). Book Detail now fetches the real book and its real chapters, and tapping one starts real synthesis of that chapter and opens the Player (ADR-0011) — the mini-player only ever appears once something's actually been chosen to play, never as a fabricated default. No app launch synthesizes anything anymore until the user asks for it. Reading/listening position isn't persisted yet — that's next.
